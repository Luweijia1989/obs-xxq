...
static bool create_d3d12_tex(bb_info &bb)
{
	...
	for (UINT i = 0; i < bb.count; i++) {
		hr = data.device11on12->CreateWrappedResource(bb.backbuffer[i], &rf11, D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_PRESENT,
...
static bool d3d12_init_11on12(ID3D12Device *device)
{
	...
	for (size_t i = 0; i < dxgi_possible_swap_queue_count; ++i) {
		hlog("d3d12_init_11on12: creating 11 device: queue=0x%" PRIX64, (uint64_t)(uintptr_t)dxgi_possible_swap_queues[i]);
		IUnknown *const queue = dxgi_possible_swap_queues[i];
		const HRESULT hr = create_11_on_12(device, 0, nullptr, 0, &queue, 1, 0, &data.device11, &data.context11, nullptr);
...
static bool d3d12_shtex_init(ID3D12Device *device, HWND window, bb_info &bb)
{
	...
	if (!capture_init_shtex(&data.shtex_info, window, data.cx, data.cy, data.format, false, (uintptr_t)data.handle)) {
		return false;
	}
	...
	if (SUCCEEDED(hr)) {
		...
		if (d3d12_init_format(swap, window, bb)) {
			...
			if (!d3d12_shtex_init(device, window, bb))
				...
				imgui_init_dx12(device, window, swap);
		}
...
static HRESULT STDMETHODCALLTYPE hook_execute_command_lists(ID3D12CommandQueue *queue, UINT NumCommandLists, ID3D12CommandList *const *ppCommandLists)
{
	...
	if (dxgi_possible_swap_queue_count < _countof(dxgi_possible_swap_queues)) {
		if ((dxgi_presenting > 0) && (queue->GetDesc().Type == D3D12_COMMAND_LIST_TYPE_DIRECT)) {
			if (try_append_queue_if_unique(queue)) {
				hlog("Remembering D3D12 queue from present: queue=0x%" PRIX64, (uint64_t)(uintptr_t)queue);
			}
		} else if (dxgi_present_attempted && (queue->GetDesc().Type == D3D12_COMMAND_LIST_TYPE_DIRECT)) {
			if (try_append_queue_if_unique(queue)) {
				hlog("Remembering D3D12 queue from first direct submit after present: queue=0x%" PRIX64, (uint64_t)(uintptr_t)queue);
			}
		} else {
