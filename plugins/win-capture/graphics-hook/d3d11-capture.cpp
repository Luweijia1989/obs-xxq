...
static bool create_d3d11_tex(uint32_t cx, uint32_t cy, ID3D11Texture2D **tex, ID3D11ShaderResourceView **resource, ID3D11RenderTargetView **render_target,
			     HANDLE *handle)
{
	UINT flags = 0;
	UINT misc_flags = 0;
	...
	if (!!resource) {
		...
		hr = data.device->CreateShaderResourceView(*tex, &res_desc, resource);
		if (FAILED(hr)) {
	...
	if (!!render_target) {
		hr = data.device->CreateRenderTargetView(*tex, nullptr, render_target);
		if (FAILED(hr)) {
...
static bool d3d11_shtex_init(HWND window)
{
	...
	success = create_d3d11_tex(data.cx, data.cy, &data.texture, &resource, &data.render_target, &data.handle);
	if (resource)
		resource->Release();
