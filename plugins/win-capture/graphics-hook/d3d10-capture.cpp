...
static bool create_d3d10_tex(uint32_t cx, uint32_t cy, ID3D10Texture2D **tex, ID3D10ShaderResourceView **resource, ID3D10RenderTargetView **render_target,
			     HANDLE *handle)
{
	UINT flags = 0;
	UINT misc_flags = 0;
...
static bool d3d10_shtex_init(HWND window)
{
	...
	success = create_d3d10_tex(data.cx, data.cy, &data.texture, &resource, &data.render_target, &data.handle);
	if (resource)
		resource->Release();
