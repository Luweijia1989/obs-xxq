...
const be_rgba_color  BE_COLOR_HEAD_MASK = { 0.1, 0.28, 0.6, 0.3 };
    ...
    for (int i = 0; i < faceInfo->face_count; i++) {
        ...
        if (withExtra) { //draw th extra face info
            ...
            if (extendInfo->eye_count > 0) {
                //left eye
                be_rgba_color color = { 200.0f / 255.0f, 0.0f, 0.0f, 1.0f };
            ...
            if (extendInfo->eyebrow_count > 0) {
                ...
                be_rgba_color color = { 220.0f / 255.0f, 0.0f, 0.0f, 1.0f };
            ...
            if (extendInfo->iris_count > 0) {
                ...
                be_rgba_color color = { 1.0f, 180.0f / 255.0f, 0.0f, 1.0f };
            ...
            if (extendInfo->lips_count > 0) {
                ...
                be_rgba_color color = { 200.0f / 255.0f, 40.0f / 255.0f, 40.0f / 255.0f, 1.0f };
...
void BERender::drawFaceRect(bef_ai_face_info * faceInfo) {
    ...
    for (int i = 0; i < faceInfo->face_count; i++) {
        //draw basic 106 rect
        bef_ai_rect* rect = &faceInfo->base_infos[i].rect;
        m_renderHelper->drawRect(rect, BE_COLOR_RED ,BE_FACE_BOX_WIDTH* m_drawRatio);
    }
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    checkGLError();
}

void BERender::drawFaceMask(bef_ai_mouth_mask_info* mouthMask, bef_ai_teeth_mask_info* teethMask, bef_ai_face_mask_info* faceMask) {
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_currentTexture, 0);
    ...
    for (int i = 0; i < faceMask->face_count; i++) {
        be_rgba_color maskColor = { 0.0f, 0.0f, 1.0f, 1.0f };
        int size[2] = { BEF_FACE_MASK_WIDTH , BEF_FACE_MASK_WIDTH };
        m_renderHelper->drawFaceSegment(faceMask->face_mask[i].mask, maskColor, m_currentTexture, faceMask->face_mask[i].warp_mat, size);
    }

    for (int i = 0; i < mouthMask->face_count; i++) {
        be_rgba_color maskColor = { 1.0f, 0.0f, 0.0f, 1.0f };
        int size[2] = { BEF_MOUTH_MASK_WIDTH , BEF_MOUTH_MASK_WIDTH };
        m_renderHelper->drawFaceSegment(mouthMask->mouth_mask[i].mask, maskColor, m_currentTexture, mouthMask->mouth_mask[i].warp_mat, size);
    }

    for (int i = 0; i < teethMask->face_count; i++) {
        be_rgba_color maskColor = { 0.0f, 1.0f, 0.0f, 1.0f };
        int size[2] = { BEF_MOUTH_MASK_WIDTH , BEF_MOUTH_MASK_WIDTH };
        m_renderHelper->drawFaceSegment(teethMask->teeth_mask[i].mask, maskColor, m_currentTexture, teethMask->teeth_mask[i].warp_mat, size);
    }
...
void BERender::drawSkeleton(bef_ai_skeleton_info *skeletonInfo, int validCount) {
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_currentTexture, 0);
    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (status != GL_FRAMEBUFFER_COMPLETE) {
    ...
    {
        bef_ai_skeleton_info *info = skeletonInfo + countIndex;
        bef_ai_rect rect= info->skeletonRect;
        m_renderHelper->drawRect(&rect, BE_COLOR_BLUE, BE_HAND_KEYPOINT_LINE_WIDTH);
        for (int pointIndex =0 ;pointIndex<BEF_AI_MAX_SKELETON_POINT_NUM;pointIndex++)
        {
             bef_ai_skeleton_point_info *point = info->keyPointInfos + pointIndex;
             if (point->is_detect)
             {
        ...
        {
            bef_ai_skeleton_point_info *left = info->keyPointInfos + pairs[lineIndex];
            bef_ai_skeleton_point_info *right = info->keyPointInfos + pairs[lineIndex + 1];
...
void BERender::drawHands(bef_ai_hand_info *handsInfo) {
    ...
    for (int i = 0; i < handsCount; i++) {
        ...
        bef_ai_tt_key_point *baiscHandKeyPoints = hand->key_points;
        ...
        bef_ai_tt_key_point *extendHandKeyPoins = hand->key_points_extension;
...
void BERender::drawHairParse(unsigned char *mask, int *maskSize) {
    ...
    m_renderHelper->drawMask(mask, m_currentTexture, BE_COLOR_HAIR, maskSize);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    checkGLError();
}
...
void BERender::drawPortrait(unsigned char *mask, int *maskSize) {
   ...
   // m_renderHelper->drawMask(mask, m_currentTexture, BE_COLOR_PRORTRAIT, maskSize);
    m_renderHelper->drawPortraitMask(mask, BE_COLOR_PRORTRAIT, m_currentTexture, maskSize);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    checkGLError();
}

void BERender::drawHeadSeg(unsigned char* mask, float* affine, int* maskSize) {
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_currentTexture, 0);

    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        printf("glCheckFramebufferStatus when draw headSeg\n");
    }
    m_renderHelper->drawMask(mask, m_currentTexture, (float*)affine, BE_COLOR_HEAD_MASK, maskSize);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    checkGLError();
}

void BERender::drawGaze(bef_ai_gaze_estimation_info* gazeInfo) {
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_currentTexture, 0);
    ...
    for (int i = 0; i < gazeInfo->face_count; i++) {
        ...
        m_renderHelper->drawLine(&lline, BE_COLOR_RED, 5.0f);
        m_renderHelper->drawLine(&rline, BE_COLOR_RED, 5.0f);
    }
