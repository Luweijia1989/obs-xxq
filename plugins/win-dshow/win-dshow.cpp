	...
	long long startTime, long long endTime)
{
	...
	if (use_face_sticker && stThread && stThread->stInited())
		...
		OutputFrame((videoConfig.format == VideoFormat::XRGB || videoConfig.format == VideoFormat::ARGB), videoConfig.format, data, size, startTime, endTime);
}

void DShowInput::OutputFrame(bool f, VideoFormat vf, unsigned char *data, size_t size, long long startTime, long long endTime)
{
	...
	if (vf == VideoFormat::XRGB ||
		vf == VideoFormat::ARGB) {
		frame.data[0] = data;
		frame.linesize[0] = cx * 4;

	}
	else if (vf == VideoFormat::YVYU ||
		vf == VideoFormat::YUY2 ||
		vf == VideoFormat::HDYC ||
		vf == VideoFormat::UYVY) {
		frame.data[0] = data;
...
static void UpdateDShowInput(void *data, obs_data_t *settings)
{
	...
	{
		input->use_face_sticker = obs_data_get_bool(settings, USE_FACE_STICKER);
		input->face_sticker_id = obs_data_get_string(settings, FACE_STICKER_ID);
