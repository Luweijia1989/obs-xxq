...
static bool libsrt_write(struct srt_output *stream, char *buf, int size)
{
	bool index = 1;
	SRTContext *s = &stream->srtContext;
	SRT_SOCKSTATUS stt = srt_getsockstate(s->fd);
	int ret = srt_sendmsg(s->fd, buf, size, -1, 0);
		       ...
		       size_t idx)
{
	...
	success = libsrt_write(stream, (char *)data, (int)size);
...
{
	struct srt_output *stream = bzalloc(sizeof(struct srt_output));
...
struct obs_output_info srt_output_info = {
	...
	.get_name = srt_output_getname,
	.create = srt_output_create,
	.destroy = srt_output_destroy,
	.start = srt_output_start,
	.stop = srt_output_stop,
	.encoded_packet = srt_output_data,
};
