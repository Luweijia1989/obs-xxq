...
GauseBlur::GauseBlur(double sigma, int dist) : m_dist(dist)
{
	m_matrix = CalcGauseBlurMatrix(sigma, dist);
}
...
UINT GauseBlur::GetGauseBlur(int x, int y, const UINT *puStart, int width,
			     const RECT &rect, double *matrix, int dist)
{
	UINT ret = 0;
	...
	for (int yn = ystart, i = max(0, dist - y); yn <= yend && i < sz;
	     ++yn, ++i)
		for (int xn = xstart, j = max(0, dist - x);
		     xn <= xend && j < sz; ++xn, ++j) {
...
void GauseBlur::Do(UINT *puStart, int width, int height)
{
	...
	for (int y = 0; y < height; ++y)
		for (int x = 0; x < width; ++x) {
			puStart[index] = GetGauseBlur(x, y, puStart, width, rc,
						      m_matrix, m_dist);
			++index;
		}
}
