 ...
 * @breif            加载当前帧资源.
 * @param handle     Effect handle
 * @param resourceLoadingTimeoutUs 等待资源加载时间，单位us。
 * timeoutUs=-1时，资源加载完成后同步返回。timeoutUs=0时，异步加载不等待。其他值为异步加载，最多等待timeout us后返回。
 * @return           if succeed return IES_RESULT_SUC,  other value please see bef_effect_base_define.h
 */
BEF_SDK_API bef_effect_result_t bef_effect_ai_load_resource_with_timeout(bef_effect_handle_t handle, int resourceLoadingTimeoutUs);
...
/// @brief 处理触摸事件，不同的 event_code 值对应着不同的事件
/// @param handle 已创建的句柄
/// @param event_code 事件类型
/// @param x x 轴触摸位置，0-1
/// @param y y 轴触摸位置，0-1
/// @param extra 如果是旋转、缩放等事件，extra 表示为旋转角度、缩放倍数等
///              当作为旋转角度表示时，顺时针旋转为负，逆时针为正
BEF_SDK_API bef_effect_result_t bef_effect_ai_process_multi_touch_event(bef_effect_handle_t handle, bef_ai_event_code event_code, float x, float y, float extra);
...
/**
 * @brief check if the license is valid
          检查license文件是否有效
 * @param licensePath licbag file path
 *                    licbag文件路径
 * @return If succeed return BEF_RESULT_SUC, other value please refer bef_effect_ai_public_define.h
 *         成功返回 BEF_RESULT_SUC, 授权码非法返回BEF_RESULT_INVALID_LICENSE，其它失败返回相应错误码, 具体请参考 bef_effect_ai_public_define.h 
 *
 */
BEF_SDK_API bef_effect_result_t
bef_effect_ai_check_license_base(const char* licensePath);
#endif

typedef int (*logAIFileFuncPointer)(int logLevel, const char* msg);

BEF_SDK_API bef_effect_result_t bef_effect_ai_set_log_to_local_func(logAIFileFuncPointer pfunc);
