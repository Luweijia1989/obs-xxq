...
static void initBDResource(std::string appPath)
{
	...
	QFileInfo finfo(getResourceContext()
				->getFeatureContext()
				->getLicensePath()
				.c_str());
	if (!finfo.exists() || finfo.size() == 0) {
		...
		QString key =
			"biz_license_tool_test_key1dc40f68d29c421e977b01d61b13f5e0";
		...
		QString temp = QString("%1%2%3%4")
				       ...
				       .arg(timestamp)
				       .arg(authMsg);
		auto data = QMessageAuthenticationCode::hash(
			temp.toUtf8(), screct.toUtf8(),
		...
		QObject::connect(reply, &QNetworkReply::finished, [=, &loop]() {
			...
			if (!o.isEmpty()) {
				...
				std::vector<uint8_t> lic_b =
					base64_decode(base64Str.toStdString());
				QFile f(getResourceContext()
						->getFeatureContext()
						->getLicensePath()
						.c_str());
				f.open(QFile::ReadWrite | QFile::Truncate);
				f.write((const char *)lic_b.data(),
					lic_b.size());
				f.close();
			}
			reply->deleteLater();
