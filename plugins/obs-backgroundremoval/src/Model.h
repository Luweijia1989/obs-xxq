...
public:
	...
	virtual void populateInputOutputNames(const std::unique_ptr<Ort::Session> &session, std::vector<const char *> &inputNames,
	...
	virtual bool populateInputOutputShapes(const std::unique_ptr<Ort::Session> &session, std::vector<std::vector<int64_t>> &inputDims,
					       std::vector<std::vector<int64_t>> &outputDims)
	{
		...
		if (inputDims[0].size() < 3 || outputDims[0].size() < 3) {
			blog(LOG_ERROR, "Input or output tensor dims are < 3. input = %d, output = %d", (int)inputDims.size(), (int)outputDims.size());
			return false;
		}
	...
	virtual void allocateTensorBuffers(const std::vector<std::vector<int64_t>> &inputDims, const std::vector<std::vector<int64_t>> &outputDims,
					   std::vector<std::vector<float>> &outputTensorValues, std::vector<std::vector<float>> &inputTensorValues,
					   std::vector<Ort::Value> &inputTensor, std::vector<Ort::Value> &outputTensor)
	{
		...
		for (size_t i = 0; i < inputDims.size(); i++) {
			...
			inputTensor.push_back(Ort::Value::CreateTensor<float>(memoryInfo, inputTensorValues[i].data(), inputTensorValues[i].size(),
		...
		for (size_t i = 0; i < outputDims.size(); i++) {
			...
			outputTensor.push_back(Ort::Value::CreateTensor<float>(memoryInfo, outputTensorValues[i].data(), outputTensorValues[i].size(),
	...
	virtual void prepareInputToNetwork(cv::Mat &resizedImage, cv::Mat &preprocessedImage) { preprocessedImage = resizedImage / 255.0; }
	...
	virtual cv::Mat getNetworkOutput(const std::vector<std::vector<int64_t>> &outputDims, std::vector<std::vector<float>> &outputTensorValues,
					 std::vector<std::vector<int64_t>> &inputDims, std::vector<std::vector<float>> &inputTensorValues)
	{
		// BHWC
	...
	virtual void runNetworkInference(const std::unique_ptr<Ort::Session> &session, const std::vector<const char *> &inputNames,
					 const std::vector<const char *> &outputNames, const std::vector<Ort::Value> &inputTensor,
					 std::vector<Ort::Value> &outputTensor)
	{
		...
		session->Run(Ort::RunOptions{nullptr},
			     ...
			     inputNames.data(), inputTensor.data(), inputNames.size(), outputNames.data(), outputTensor.data(), outputNames.size());
	}
};
...
public:
	...
	virtual void loadInputToTensor(const cv::Mat &preprocessedImage, uint32_t inputWidth, uint32_t inputHeight,
				       std::vector<std::vector<float>> &inputTensorValues)
	{
		inputTensorValues[0].assign(preprocessedImage.begin<float>(), preprocessedImage.begin<float>() + inputWidth * inputHeight * 3);
