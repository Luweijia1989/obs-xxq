...
WebCapture::WebCapture(QString memoryName) : m_source(nullptr), m_imageTexture(nullptr), width(0), height(0) {}
...
void WebCapture::updateTextureData(obs_data_t *command)
{
	...
	if (w != width || h != height || !obs_data_has_user_value(command, "hScale") || !obs_data_has_user_value(command, "wScale")) {
...
{
	WebCapture *capture = new WebCapture(obs_data_get_string(settings, "memroyName"));
...
static void webcapture_source_render(void *data, gs_effect_t *effect)
{
	...
	gs_effect_set_texture(gs_effect_get_param_by_name(effect, "image"), capture->m_imageTexture);
