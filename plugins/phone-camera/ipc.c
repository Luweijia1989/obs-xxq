...
void ipc_server_create(struct IPCServer **input, const char *name, read_cb cb, void *param)
{
	*input = (struct IPCServer *)calloc(1, sizeof(struct IPCServer));
	...
	sprintf_s(server->m_sAddr, IPC_MAX_ADDR, "%s_%s", IPC_MEMORY_NAME, name);
	server->cb = cb;
	server->m_cbParam = param;

	server->m_workingMutex = CreateMutexA(NULL, false, server->m_sAddr);
	if (server->m_workingMutex == NULL || server->m_workingMutex ==  INVALID_HANDLE_VALUE)
		return;
...
void ipc_server_destroy(struct IPCServer **input)
{
	...
	if (server->m_workingMutex) {
		CloseHandle(server->m_workingMutex);
		server->m_workingMutex = NULL;
	}
...
static void ipc_client_create_private(struct IPCClientPrivate **input, const char *name)
{
	*input = (struct IPCClientPrivate *)calloc(1, sizeof(struct IPCClientPrivate));
	struct IPCClientPrivate *client = *input;

	sprintf_s(client->m_sAddr, IPC_MAX_ADDR, "%s_%s", IPC_MEMORY_NAME, name);
...
static int ipc_client_write_private(struct IPCClientPrivate *client, void *pBuff, DWORD amount,
		       DWORD dwTimeout)
{
	if(!client)
		return 0;

	HANDLE handle = OpenMutexA(MUTEX_ALL_ACCESS, false, client->m_sAddr);
	if (handle == NULL) {
		return -1;
	}
...
void ipc_client_create(struct IPCClient **input, const char *name)
{
	*input = (struct IPCClient *)calloc(1, sizeof(struct IPCClient));
	...
	ipc_client_create_private(&client->m_private, name);
}
...
{
	EnterCriticalSection(&client->m_mutex);
	int ret = ipc_client_write_private(client->m_private, pBuff, amount, dwTimeout);
	LeaveCriticalSection(&client->m_mutex);
	return ret;
}
