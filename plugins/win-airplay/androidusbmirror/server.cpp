...
static bool disable_tunnel(struct server *server)
{
	if (server->tunnel_forward) {
		return disable_tunnel_forward(server->serial,
					      server->local_port);
	}
...
static bool enable_tunnel_reverse_any_port(struct server *server,
					   struct port_range port_range)
{
	...
	for (;;) {
		...
		if (!disable_tunnel_reverse(server->serial)) {
			LOGW("Could not remove reverse tunnel on port %" PRIu16,
			     port);
		}

		// check before incrementing to avoid overflow on port 65535
		if (port < port_range.last) {
			LOGW("Could not listen on port %" PRIu16
			     ", retrying on %" PRIu16,
		...
		if (port_range.first == port_range.last) {
			LOGE("Could not listen on port %" PRIu16,
			     port_range.first);
		} else {
			LOGE("Could not listen on any port in range %" PRIu16
			     ":%" PRIu16,
...
static bool enable_tunnel_forward_any_port(struct server *server,
					   struct port_range port_range)
{
	...
	for (;;) {
		...
		if (port < port_range.last) {
			LOGW("Could not forward port %" PRIu16
			     ", retrying on %" PRIu16,
			     port, port + 1);
			port++;
			continue;
		...
		if (port_range.first == port_range.last) {
			LOGE("Could not forward port %" PRIu16,
			     port_range.first);
		} else {
			LOGE("Could not forward any port in range %" PRIu16
			     ":%" PRIu16,
			     port_range.first, port_range.last);
		}
		return false;
...
static bool enable_tunnel_any_port(struct server *server,
				   struct port_range port_range,
...
static process_t execute_server(struct server *server,
...
static socket_t connect_to_server(uint16_t port, uint32_t attempts,
				  uint32_t delay)
{
	do {
...
void server_init(struct server *server)
{
	...
	server->server_socket_closed.clear();
		  ...
		  const struct server_params *params)
{
	...
	if (pthread_create(&server->wait_server_thread, NULL, run_wait_server,
			   server) != 0) {
...
void server_stop(struct server *server)
{
	...
	pthread_join(server->wait_server_thread, NULL);
}
