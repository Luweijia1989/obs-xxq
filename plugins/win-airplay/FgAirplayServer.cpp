...
void FgAirplayServer::connected(void *cls, const char *remoteName,
...
void FgAirplayServer::disconnected(void *cls, const char *remoteName,
				   const char *remoteDeviceId)
{
	FgAirplayServer *pServer = (FgAirplayServer *)cls;
	if (!pServer) {
...
void FgAirplayServer::audio_set_metadata(void *cls, void *session,
...
void FgAirplayServer::audio_set_coverart(void *cls, void *session,
					 const void *buffer, int buflen,
					 const char *remoteName,
					 const char *remoteDeviceId)
{
...
void FgAirplayServer::ap_video_get_play_info(void *cls, double *duration,
					     double *position, double *rate)
{
	...
	if (pServer->m_pCallback) {
		pServer->m_pCallback->videoGetPlayInfo(duration, position,
						       rate);
	}
}
