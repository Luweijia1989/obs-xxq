...
static int send_packet(struct mux_device *dev, enum mux_protocol proto, void *header, const void *data, int length)
{
	...
	if((res = usb_bulk_write(dev->dev_handle, dev->ep_out , buffer, total, 1000)) < 0) {
...
static void device_version_input(struct mux_device *dev, struct version_header *vh)
{
	...
	usbmuxd_log(LL_NOTICE, "Connected to v%d.%d device %d on location 0x%x with serial number %s", dev->version, vh->minor, dev->id, dev->device->bus->location, dev->serial);
	dev->state = MUXDEV_ACTIVE;
	collection_init(&dev->connections);
	struct device_info info;
	info.id = dev->id;
	info.location = dev->device->bus->location;
	info.serial = dev->serial;
	info.pid = dev->device->descriptor.idProduct;
	info.speed = 480000000;
 ...
 * @param length
 */
void device_data_input(usb_dev_handle *handle, unsigned char *buffer, uint32_t length)
{
	struct mux_device *dev = NULL;
	pthread_mutex_lock(&device_list_mutex);
	FOREACH(struct mux_device *tdev, &device_list) {
		if(tdev->dev_handle == handle) {
			dev = tdev;
			break;
		}
	} ENDFOREACH
	pthread_mutex_unlock(&device_list_mutex);
	if(!dev) {
		usbmuxd_log(LL_WARNING, "Cannot find device entry for RX input from USB device %p on location 0x%x", dev->dev_handle, dev->device->bus->location);
		return;
	}
...
int device_add(struct usb_device *device, usb_dev_handle *handle, int ep_out, char *serial)
{
	int res;
	int id = get_next_device_id();
	struct mux_device *dev;
	usbmuxd_log(LL_NOTICE, "Connecting to new device on location 0x%x as ID %d",device->bus->location, id);
...
void device_remove(usb_dev_handle *handle)
{
	pthread_mutex_lock(&device_list_mutex);
	FOREACH(struct mux_device *dev, &device_list) {
		if(dev->dev_handle == handle) {
			usbmuxd_log(LL_NOTICE, "Removed device %d", dev->id);
			if(dev->state == MUXDEV_ACTIVE) {
	...
	usbmuxd_log(LL_WARNING, "Cannot find device entry while removing USB device %p", handle);
}
	...
	*devices = malloc(sizeof(struct device_info) * dev_list.capacity);
	...
	FOREACH(struct mux_device *dev, &device_list) {
		if((dev->state == MUXDEV_ACTIVE) && (include_hidden || dev->visible)) {
			p->id = dev->id;
			p->serial = dev->serial;
			p->location = dev->device->bus->location;
			p->pid = dev->device->descriptor.idProduct;
			p->speed = 480000000;
			count++;
			p++;
		}
