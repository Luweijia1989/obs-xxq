...
static void handle_sync_packet(unsigned char *buf, uint32_t length)
{
	...
	case CWPA:
	{
		...
		app_device.mp.localAudioClock = NewCMClockWithHostTime(clockRef);
		...
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, device_info, device_info_len, 1000);

		uint8_t *cwpa_reply;
		size_t cwpa_reply_len;
		CwpaPacketNewReply(&cwpaPacket, clockRef, &cwpa_reply, &cwpa_reply_len);
		usbmuxd_log(LL_NOTICE, "Send CWPA-RPLY {correlation:%p, clockRef:%p}", cwpaPacket.CorrelationID, clockRef);
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, cwpa_reply, cwpa_reply_len, 1000);
		free(cwpa_reply);
		...
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, hpa1, hpa1_len, 1000);
		free(hpa1);
	}
	break;
	case AFMT:
	{
		...
		usbmuxd_log(LL_NOTICE, "Send AFMT-REPLY {correlation:%p}", afmtPacket.CorrelationID);
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, afmt, afmt_len, 1000);
		free(afmt);
	}
	break;
	case CVRP:
	{
		...
		AsynNeedPacketBytes(app_device.mp.needClockRef, &app_device.mp.needMessage, &app_device.mp.needMessageLen);
		...
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, send_data, send_data_len, 1000);
		free(send_data);
	...
	case SKEW:
	{
		...
		double skewValue = CalculateSkew(&app_device.mp.startTimeLocalAudioClock,
		...
		uint8_t *send_data;
		size_t send_data_len;
		SyncSkewPacketNewReply(&skew_packet, skewValue, &send_data, &send_data_len);
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, send_data, send_data_len, 1000);
		free(send_data);
	}
	break;
	...
	break;
	default:
		usbmuxd_log(LL_INFO, "received unknown sync packet type");//stop
		break;
	}
...
static void handle_async_packet(unsigned char *buf, uint32_t length)
{
	...
	case EAT:
	{
		...
		else
		{
			...
			{
				app_device.mp.startTimeDeviceAudioClock = eatPacket.CMSampleBuf.OutputPresentationTimestamp;
				app_device.mp.startTimeLocalAudioClock = GetTime(&app_device.mp.localAudioClock);
				app_device.mp.lastEatFrameReceivedDeviceAudioClockTime = eatPacket.CMSampleBuf.OutputPresentationTimestamp;
				app_device.mp.lastEatFrameReceivedLocalAudioClockTime = app_device.mp.startTimeLocalAudioClock;
				app_device.mp.firstAudioTimeTaken = true;
			}
			else
			{
				app_device.mp.lastEatFrameReceivedDeviceAudioClockTime = eatPacket.CMSampleBuf.OutputPresentationTimestamp;
				app_device.mp.lastEatFrameReceivedLocalAudioClockTime = GetTime(&app_device.mp.localAudioClock);
			}

			app_device.mp.consumer.consume(&eatPacket.CMSampleBuf, app_device.mp.consumer.consumer_ctx);

			if (app_device.mp.audioSamplesReceived % 100 == 0)
			{
				usbmuxd_log(LL_INFO, "RCV Audio Samples:%d", app_device.mp.audioSamplesReceived);
			}
	...
	case FEED:
	{
		...
		else
		{
			...
			app_device.mp.consumer.consume(&acsbp.CMSampleBuf, app_device.mp.consumer.consumer_ctx);

			if (app_device.mp.videoSamplesReceived % 500 == 0)
			{
				usbmuxd_log(LL_INFO, "Rcv'd(%d) ", app_device.mp.videoSamplesReceived);
...
static void frame_received(unsigned char *buf, uint32_t length)
{
	...
	case PingPacketMagic:
		if (!app_device.first_ping_packet)
		{
			...
			usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, cmd, 16, 1000);
...
static void usb_extract_frame(unsigned char *buf, uint32_t length)
{
	...
	while (true)
	{
		size_t byte_remain = ringbuf_bytes_used(app_device.usb_data_buffer);
		if (byte_remain < 4)
			break;

		unsigned char buff_len[4] = { 0 };
		ringbuf_memcpy_from_not_remove(buff_len, app_device.usb_data_buffer, 4);
		size_t len = byteutils_get_int(buff_len, 0);
		if (byte_remain >= len)
		{
			//有一帧
			uint8_t *temp_buf = calloc(1, len);
			ringbuf_memcpy_from(temp_buf, app_device.usb_data_buffer, len);
...
void *usb_read_thread(void *data)
{
	...
	{
		read_len = usb_bulk_read(app_device.device_handle, app_device.ep_in_fa, read_buffer, 65536, 0);
		if (read_len < 0)
		{
			break;
		}
...
void *usb_read_lock_down_thread(void *data)
{
	...
	{
		read_len = usb_bulk_read(app_device.device_handle, app_device.ep_in, read_buffer, 65536, 100);
		if (read_len < 0)
		{
			if (read_len != -116)
			{
		...
		device_data_input(app_device.device_handle, read_buffer, read_len);
	}
	return NULL;
}
...
void closeSession()
{
		...
		return;
	{
		...
		usb_bulk_write(app_device.device_handle, app_device.ep_out_fa, d1, d1_len, 1000);
		free(d1);
	}
	...
	usb_control_msg(app_device.device_handle, 0x40, 0x52, 0x00, 0x00, NULL, 0, 1000 /* LIBUSB_DEFAULT_TIMEOUT */);
...
int usb_device_discover()
{
	...
	else
	{
		...
		else
		{
			if (app_device.device_handle)
				reset_app_device();
			app_device.stop_signal = CreateEvent(NULL, false, false, NULL);
			app_device.device = device;
			app_device.device_handle = handle;
			app_device.usb_data_buffer = ringbuf_new(1024 * 1024 * 2);
			app_device.mp.consumer = AVFileWriter("out.h264", "out.wav");
			in_progress = true;
			res = 1;
		}
	}
...
int main(void)
{
	...
	while (!should_exit)
	{
		...
		{
			usb_get_string_simple(app_device.device_handle,
				app_device.device->descriptor.iSerialNumber,
				app_device.serial,
				sizeof(app_device.serial));

			int muxConfigIndex = -1;
			int qtConfigIndex = -1;

			findConfigurations(&app_device.device->descriptor, app_device.device, &muxConfigIndex, &qtConfigIndex);

			if (qtConfigIndex == -1)
			{
				int res = usb_control_msg(app_device.device_handle, 0x40, 0x52, 0x00, 0x02, NULL, 0, 5000 /* LIBUSB_DEFAULT_TIMEOUT */);
				Sleep(300);
				continue;
			}
			else
			{
				/*if (app_device.has_reset == 0)
				{
					usb_reset(app_device.device_handle);
					reset_app_device();
					app_device.has_reset = 1;
					continue;
				}*/
			}

			byte config = -1;
			int res = usb_control_msg(app_device.device_handle, USB_RECIP_DEVICE | USB_ENDPOINT_IN, USB_REQ_GET_CONFIGURATION, 0, 0, &config, 1, 5000 /* LIBUSB_DEFAULT_TIMEOUT */);
			if (config != qtConfigIndex)
			{
				usb_set_configuration(app_device.device_handle, qtConfigIndex);
				//continue;
			}
			...
			if (perr != 0) {
				usbmuxd_log(LL_ERROR, "ERROR: failed to start usb read thread for device %s: %s (%d)", app_device.serial, strerror(perr), perr);
			}

			/*	pthread_t ddf;
				perr = pthread_create(&ddf, NULL, testdd, NULL);*/
		}
	}
