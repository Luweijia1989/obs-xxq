...
//}
//
//void usb_win32_control_cmd(const char serial[], int requesttype, int request, int value, int index, char *bytes, int size, int timeout)
//{
//	usbmuxd_log(LL_INFO, "Getting the configuration for device %s using libusb-win32", serial);
//
//	usb_dev_handle* device = usb_win32_open(serial);
...
//
//	byte config = -1;
//	int res = usb_control_msg(device, requesttype, request, value, index, NULL, 0, 5000 /* LIBUSB_DEFAULT_TIMEOUT */);
//
//	if (res < 0) {
//		usbmuxd_log(LL_ERROR, "Could not get the configuration for device %s using libusb-win32: %d", serial, res);
//		return res;
