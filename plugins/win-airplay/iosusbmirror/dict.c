...
void PutStringKeyNSNumber2(struct StringKeyEntry *entry, uint8_t *key, size_t key_len, struct NSNumber *value)
{
	...
	entry->serialized_data = calloc(1, entry->data_len);
	WriteLengthAndMagic(entry->serialized_data, entry->data_len, NumberValueMagic);
...
void PutStringKeyString2(struct StringKeyEntry *entry, uint8_t *key, size_t key_len, uint8_t *value, size_t value_len)
{
	...
	entry->serialized_data = calloc(1, entry->data_len);
	WriteLengthAndMagic(entry->serialized_data, entry->data_len, StringValueMagic);
...
void PutStringKeyBytes2(struct StringKeyEntry *entry, uint8_t *key, size_t key_len, uint8_t *bytes, size_t bytes_len)
{
	...
	entry->serialized_data = calloc(1, entry->data_len);
	WriteLengthAndMagic(entry->serialized_data, entry->data_len, DataValueMagic);
...
list_t * NewStringDictFromBytes(uint8_t *bytes, size_t bytes_len)
{
	...
	size_t value_len;
	if (ParseLengthAndMagic(bytes, bytes_len, DictionaryMagic, &remaining_bytes, &value_len) < 0)
		return NULL;
	...
	while (slice < bytes + bytes_len)
	{
		...
		uint8_t *p;
		if (ParseLengthAndMagic(slice, bytes_len -(slice-bytes), KeyValuePairMagic, &p, &keyValuePairLength) < 0)
		{
			has_error = true;
			break;
		}

		struct list_node *node = parseEntry(slice+8, keyValuePairLength-8);
		if (!node)
		{
			has_error = true;
			break;
...
{
	return NewIndexDictFromBytesWithCustomMarker(bytes, bytes_len, DictionaryMagic);
}
...
list_t * NewIndexDictFromBytesWithCustomMarker(uint8_t *bytes, size_t bytes_len, uint32_t magic)
{
	uint8_t *remaining_bytes = NULL;
	size_t value_len;
	if (ParseLengthAndMagic(bytes, bytes_len, magic, &remaining_bytes, &value_len) < 0)
		return NULL;
	...
	while (slice < bytes + bytes_len)
	{
		...
		struct list_node *node = parseIntDictEntry(slice + 8, keyValuePairLength - 8);
		if (!node)
		{
			has_error = true;
			break;
...
struct list_node *parseIntDictEntry(uint8_t *data, size_t data_len)
{
	...
	if (parseIntValue(remaining_bytes, data_len-(remaining_bytes-data), key, entry) < 0)
	{
		free(entry);
		return NULL;
	}
...
int parseValue(uint8_t *data, size_t data_len, uint8_t *key, size_t key_len, struct StringKeyEntry *entry)
{
	...
	case StringValueMagic:
		PutStringKeyString2(entry, key, key_len, data+8, value_len-8);
		break;
	case DataValueMagic:
		PutStringKeyBytes2(entry, key, key_len, data+8, value_len-8);
		break;
	...
	{
		struct FormatDescriptor *fd = calloc(1, sizeof(struct FormatDescriptor));
...
struct list_node *parseValueEntry(uint8_t *data, size_t data_len)
{
	...
	size_t key_len;
	if (ParseLengthAndMagic(data, data_len, KeyValuePairMagic, &next_data_pointer, &key_len) < 0)
		return NULL;
...
struct list_node * parseEntry(uint8_t *data, size_t data_len)
{
	...
	if (parseValue(remaining_data, data_len-(remaining_data-data), key, key_len, entry) < 0)
	{
		free(entry);
		return NULL;
	}
...
void SerializeStringKeyDict(list_t *stringKeyDict, uint8_t **out, size_t *out_len)
{
	...
	while ((node = list_iterator_next(it))) {
		...
		if (entry->typeMagic != format_descriptor_type)
		{
			...
			{
				SerializeStringKeyDict(entry->children, &entry->serialized_data, &entry->data_len);
				memcpy(keyvaluePair + keyLength, entry->serialized_data, entry->data_len);
		...
		WriteLengthAndMagic(slice + index, keyLength + entry->data_len + 8, KeyValuePairMagic);
	...
	*out_len = dictSizePlusHeaderAndLength;
	WriteLengthAndMagic(temp_buf, dictSizePlusHeaderAndLength, DictionaryMagic);
...
list_t * CreateHpd1DeviceInfoDict()
{
	...
	list_t *sub = list_new();
	{
		struct StringKeyEntry *subone = calloc(1, sizeof(struct StringKeyEntry));
		...
		struct StringKeyEntry *subtwo = calloc(1, sizeof(struct StringKeyEntry));
...
list_t * CreateHpa1DeviceInfoDict()
{
	...
	struct AudioStreamBasicDescription asbd = DefaultAudioStreamBasicDescription();
	SerializeAudioStreamBasicDescription(&asbd, &asbd_bytes, &asbd_bytes_len);
