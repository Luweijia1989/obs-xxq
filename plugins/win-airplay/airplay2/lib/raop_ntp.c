 ...
 */
static int raop_ntp_compare(const void *av, const void *bv)
{
...
static int raop_ntp_parse_remote_address(raop_ntp_t *raop_ntp,
					 ...
					 int remote_addr_len)
{
	...
	sprintf(current, "%d.%d.%d.%d", remote_addr[0], remote_addr[1],
		remote_addr[2], remote_addr[3]);
	logger_log(raop_ntp->logger, LOGGER_DEBUG,
		   "raop_ntp parse remote ip = %s", current);
	ret = netutils_parse_address(family, current, &raop_ntp->remote_saddr,
...
raop_ntp_t *raop_ntp_init(logger_t *logger, const unsigned char *remote_addr,
			  int remote_addr_len, unsigned short timing_rport)
{
	...
	if (raop_ntp_parse_remote_address(raop_ntp, remote_addr,
					  remote_addr_len) < 0) {
		free(raop_ntp);
		return NULL;
	}
	...
	((struct sockaddr_in *)&raop_ntp->remote_saddr)->sin_port =
		htons(timing_rport);
...
void raop_ntp_destroy(raop_ntp_t *raop_ntp)
{
	if (raop_ntp) {
...
static int raop_ntp_init_socket(raop_ntp_t *raop_ntp, int use_ipv6)
{
	int tsock = -1;
...
static THREAD_RETVAL raop_ntp_thread(void *arg)
{
	raop_ntp_t *raop_ntp = arg;
	assert(raop_ntp);
	unsigned char response[128];
	int response_len;
	unsigned char request[32] = {0x80, 0xd2, 0x00, 0x07, 0x00, 0x00, 0x00,
				     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				     0x00, 0x00, 0x00, 0x00};
	raop_ntp_data_t data_sorted[RAOP_NTP_DATA_COUNT];
	const unsigned two_pow_n[RAOP_NTP_DATA_COUNT] = {2,  4,  8,   16,
							 32, 64, 128, 256};
	...
	while (1) {
		...
		int send_len = sendto(
			raop_ntp->tsock, (char *)request, sizeof(request), 0,
			(struct sockaddr *)&raop_ntp->remote_saddr,
			raop_ntp->remote_saddr_len);
		logger_log(raop_ntp->logger, LOGGER_DEBUG,
			   "raop_ntp send_len = %d", send_len);
		if (send_len < 0) {
			logger_log(raop_ntp->logger, LOGGER_ERR,
				   "raop_ntp error sending request");
			break;
		}

		// Read response
		response_len = recvfrom(
			raop_ntp->tsock, (char *)response, sizeof(response), 0,
			(struct sockaddr *)&raop_ntp->remote_saddr,
			&raop_ntp->remote_saddr_len);
		if (response_len < 0) {
			logger_log(raop_ntp->logger, LOGGER_ERR,
		...
		logger_log(raop_ntp->logger, LOGGER_DEBUG,
			   "raop_ntp receive time type_t packetlen = %d",
			   response_len);
		...
		raop_ntp->data_index =
			(raop_ntp->data_index + 1) % RAOP_NTP_DATA_COUNT;
		raop_ntp->data[raop_ntp->data_index].time = t3;
		raop_ntp->data[raop_ntp->data_index].offset =
			((t1 - t0) + (t2 - t3)) / 2;
		raop_ntp->data[raop_ntp->data_index].delay =
			((t3 - t0) - (t2 - t1));
		raop_ntp->data[raop_ntp->data_index].dispersion =
			RAOP_NTP_R_RHO + RAOP_NTP_S_RHO +
			(t3 - t0) * RAOP_NTP_PHI_PPM / 1000000u;

		// Sort by delay
		memcpy(data_sorted, raop_ntp->data, sizeof(data_sorted));
		qsort(data_sorted, RAOP_NTP_DATA_COUNT, sizeof(data_sorted[0]),
		      raop_ntp_compare);
		...
		for (int i = 0; i < RAOP_NTP_DATA_COUNT; ++i) {
			unsigned long long disp =
				raop_ntp->data[i].dispersion +
				(t3 - raop_ntp->data[i].time) *
					RAOP_NTP_PHI_PPM / 1000000u;
		...
		logger_log(raop_ntp->logger, LOGGER_DEBUG,
			   "raop_ntp sync correction = %lld", correction);
...
void raop_ntp_start(raop_ntp_t *raop_ntp, unsigned short *timing_lport)
{
	...
	if (raop_ntp_init_socket(raop_ntp, use_ipv6) < 0) {
		logger_log(raop_ntp->logger, LOGGER_ERR,
			   "raop_ntp initializing timing socket failed");
		MUTEX_UNLOCK(raop_ntp->run_mutex);
		return;
	}
	if (timing_lport)
 ...
 * same.
 */
uint64_t raop_ntp_timestamp_to_micro_seconds(uint64_t ntp_timestamp,
					     bool account_for_epoch_diff)
{
	uint64_t seconds =
		((ntp_timestamp >> 32) & 0xffffffff) -
		(account_for_epoch_diff ? SECONDS_FROM_1900_TO_1970 : 0);
...
uint64_t raop_ntp_get_local_time(raop_ntp_t *raop_ntp)
{
	...
	clock_gettime_realtime(&time);
	return (uint64_t)time.tv_sec * 1000000L +
	       (uint64_t)(time.tv_nsec / 1000);
}
...
uint64_t raop_ntp_get_remote_time(raop_ntp_t *raop_ntp)
{
	...
	MUTEX_UNLOCK(raop_ntp->sync_params_mutex);
	return (uint64_t)((int64_t)raop_ntp_get_local_time(raop_ntp)) +
	       ((int64_t)offset);
}
 ...
 */
uint64_t raop_ntp_convert_remote_time(raop_ntp_t *raop_ntp,
