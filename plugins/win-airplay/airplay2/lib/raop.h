...
#define raop_log_debug(raop, fmt, ...) \
	raop_log(raop, RAOP_LOG_DEBUG, fmt, ##__VA_ARGS__)
#define raop_log_info(raop, fmt, ...) \
	raop_log(raop, RAOP_LOG_INFO, fmt, ##__VA_ARGS__)
#define raop_log_warn(raop, fmt, ...) \
	raop_log(raop, RAOP_LOG_WARNING, fmt, ##__VA_ARGS__)
#define raop_log_err(raop, fmt, ...) \
	raop_log(raop, RAOP_LOG_ERR, fmt, ##__VA_ARGS__)
...
struct raop_callbacks_s {
	...
	void (*connected)(void *cls, const char *remoteName,
	...
	void (*audio_flush)(void *cls, void *session, const char *remoteName,
			    const char *remoteDeviceId);
	void (*video_flush)(void *cls);
	void (*audio_set_volume)(void *cls, void *session, float volume,
				 const char *remoteName,
				 const char *remoteDeviceId);
	void (*audio_set_metadata)(void *cls, void *session, const void *buffer,
				   int buflen, const char *remoteName,
				   const char *remoteDeviceId);
	void (*audio_set_coverart)(void *cls, void *session, const void *buffer,
				   int buflen, const char *remoteName,
				   const char *remoteDeviceId);
	void (*audio_remote_control_id)(void *cls, const char *dacp_id,
					...
					const char *remoteName,
					const char *remoteDeviceId);
	void (*audio_set_progress)(void *cls, void *session, unsigned int start,
