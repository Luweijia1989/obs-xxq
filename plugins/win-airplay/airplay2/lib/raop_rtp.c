...
raop_rtp_t *raop_rtp_init(logger_t *logger, raop_callbacks_t *callbacks,
			  ...
			  const unsigned char *ecdh_secret)
{
	...
	if (remoteName != NULL) {
		strncpy(raop_rtp->remoteName, remoteName,
			min(128, strlen(remoteName)));
	}
	if (remoteDeviceId != NULL) {
		strncpy(raop_rtp->remoteDeviceId, remoteDeviceId,
...
static int raop_rtp_resend_callback(void *opaque, unsigned short seqnum,
				    unsigned short count)
{
	...
	logger_log(raop_rtp->logger, LOGGER_DEBUG,
		   "raop_rtp got resend request %d %d", seqnum, count);
	...
	ret = sendto(raop_rtp->csock, (const char *)packet, sizeof(packet), 0,
		     addr, addrlen);
	if (ret == -1) {
		logger_log(raop_rtp->logger, LOGGER_WARNING,
			   "raop_rtp resend failed: %d", SOCKET_GET_ERROR());
	}

	return 0;
}

static int raop_rtp_init_sockets(raop_rtp_t *raop_rtp, int use_ipv6,
				 int use_udp)
{
	int csock = -1, dsock = -1;
...
static int raop_rtp_process_events(raop_rtp_t *raop_rtp, void *cb_data)
{
	int flush;
	float volume;
	int volume_changed;
	...
	if (volume_changed) {
		...
		if (raop_rtp->callbacks.audio_set_volume) {
			raop_rtp->callbacks.audio_set_volume(
				raop_rtp->callbacks.cls, cb_data, volume,
				raop_rtp->remoteName, raop_rtp->remoteDeviceId);
		}
	}
	...
	if (flush != NO_FLUSH) {
		if (raop_rtp->callbacks.audio_flush) {
			raop_rtp->callbacks.audio_flush(
				raop_rtp->callbacks.cls, cb_data,
				raop_rtp->remoteName, raop_rtp->remoteDeviceId);
		}
	}
	...
	if (metadata != NULL) {
		if (raop_rtp->callbacks.audio_set_metadata) {
			raop_rtp->callbacks.audio_set_metadata(
				raop_rtp->callbacks.cls, cb_data, metadata,
				metadata_len, raop_rtp->remoteName,
				raop_rtp->remoteDeviceId);
		}
		free(metadata);
		metadata = NULL;
	...
	if (coverart != NULL) {
		if (raop_rtp->callbacks.audio_set_coverart) {
			raop_rtp->callbacks.audio_set_coverart(
				raop_rtp->callbacks.cls, cb_data, coverart,
				coverart_len, raop_rtp->remoteName,
				raop_rtp->remoteDeviceId);
		}
		free(coverart);
		coverart = NULL;
	...
	if (dacp_id && active_remote_header) {
		if (raop_rtp->callbacks.audio_remote_control_id) {
			raop_rtp->callbacks.audio_remote_control_id(
				raop_rtp->callbacks.cls, dacp_id,
				active_remote_header, raop_rtp->remoteName,
				raop_rtp->remoteDeviceId);
		}
		free(dacp_id);
	...
	if (progress_changed) {
		if (raop_rtp->callbacks.audio_set_progress) {
			raop_rtp->callbacks.audio_set_progress(
				raop_rtp->callbacks.cls, cb_data,
				progress_start, progress_curr, progress_end,
				raop_rtp->remoteName, raop_rtp->remoteDeviceId);
		}
	}
	return 0;
...
void raop_rtp_sync_clock(raop_rtp_t *raop_rtp, uint32_t rtp_time,
			 uint64_t ntp_time)
{
	raop_rtp->sync_data_index =
	...
	for (int i = 0; i < RAOP_RTP_SYNC_DATA_COUNT; ++i) {
		if (raop_rtp->sync_data[i].ntp_time == 0)
			continue;
		total_offsets +=
			(int64_t)(((double)raop_rtp->sync_data[i].rtp_time) /
				  raop_rtp->rtp_sync_scale) -
			raop_rtp->sync_data[i].ntp_time;
		valid_data_count++;
	}
	...
	logger_log(raop_rtp->logger, LOGGER_DEBUG,
		   "raop_rtp sync correction=%lld", correction);
}

uint64_t raop_rtp_convert_rtp_time(raop_rtp_t *raop_rtp, uint32_t rtp_time)
{
	return (uint64_t)(((double)rtp_time) / raop_rtp->rtp_sync_scale) -
	...
	while (1) {
		...
		if (ret == 0) {
			...
			logger_log(raop_rtp->logger, LOGGER_ERR,
				   "raop_rtp error in select");
			break;
		}

		if (FD_ISSET(raop_rtp->csock, &rfds)) {
			saddrlen = sizeof(saddr);
			packetlen = recvfrom(raop_rtp->csock, (char *)packet,
					     sizeof(packet), 0,
			...
			logger_log(raop_rtp->logger, LOGGER_DEBUG,
				   "raop_rtp type_c 0x%02x, packetlen = %d",
				   type_c, packetlen);
			if (type_c == 0x56) {
				/* Handle resent data packet */
				uint32_t rtp_timestamp = (packet[4 + 4] << 24) |
							 (packet[4 + 5] << 16) |
							 (packet[4 + 6] << 8) |
							 packet[4 + 7];
				uint64_t ntp_timestamp =
					raop_rtp_convert_rtp_time(
						raop_rtp, rtp_timestamp);
				uint64_t ntp_now =
					raop_ntp_get_local_time(raop_rtp->ntp);
				logger_log(
					raop_rtp->logger, LOGGER_DEBUG,
					"raop_rtp audio resent: ntp = %llu, now = %llu, latency=%lld, rtp=%u",
				...
				uint32_t sync_rtp =
					byteutils_get_int_be(packet, 4) - 11025;
				uint64_t sync_ntp_raw =
					byteutils_get_long_be(packet, 8);
				uint64_t sync_ntp_remote =
					raop_ntp_timestamp_to_micro_seconds(
						sync_ntp_raw, true);
				uint64_t sync_ntp_local =
					raop_ntp_convert_remote_time(
						raop_rtp->ntp, sync_ntp_remote);
				// It's not clear what the additional rtp timestamp indicates
				uint32_t next_rtp =
					byteutils_get_int_be(packet, 16);
				logger_log(
					raop_rtp->logger, LOGGER_DEBUG,
					"raop_rtp sync: ntp=%llu, local ntp: %llu, rtp=%u, rtp_next=%u",
					sync_ntp_remote, sync_ntp_local,
					sync_rtp, next_rtp);
				raop_rtp_sync_clock(raop_rtp, sync_rtp,
						    sync_ntp_local);
			} else {
				logger_log(raop_rtp->logger, LOGGER_DEBUG,
		...
		if (FD_ISSET(raop_rtp->dsock, &rfds)) {
			...
			saddrlen = sizeof(saddr);
			packetlen = recvfrom(raop_rtp->dsock, (char *)packet,
					     sizeof(packet), 0,
					     (struct sockaddr *)&saddr,
					     &saddrlen);
			// rtp payload type
			...
			if (packetlen >= 12) {
				int no_resend =
					(raop_rtp->control_rport == 0); // false
...
void raop_rtp_start_audio(raop_rtp_t *raop_rtp, int use_udp,
			  unsigned short control_rport,
			  unsigned short *control_lport,
			  unsigned short *data_lport)
{
...
void raop_rtp_remote_control_id(raop_rtp_t *raop_rtp, const char *dacp_id,
...
void raop_rtp_set_progress(raop_rtp_t *raop_rtp, unsigned int start,
