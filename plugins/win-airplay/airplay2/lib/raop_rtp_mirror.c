...
static int raop_rtp_parse_remote(raop_rtp_mirror_t *raop_rtp_mirror,
				 const unsigned char *remote, int remotelen)
{
	...
	logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
		   "raop_rtp_mirror parse remote ip = %s", current);
...
#define NO_FLUSH (-42)
raop_rtp_mirror_t *
raop_rtp_mirror_init(logger_t *logger, raop_callbacks_t *callbacks,
		     raop_ntp_t *ntp, const unsigned char *remote,
		     int remotelen, const char *remoteName,
		     const char *remoteDeviceId, const unsigned char *aeskey,
		     const unsigned char *ecdh_secret)
{
	...
	memcpy(&raop_rtp_mirror->callbacks, callbacks,
	       sizeof(raop_callbacks_t));
	raop_rtp_mirror->buffer =
		mirror_buffer_init(logger, aeskey, ecdh_secret);
	...
	if (remoteName != NULL) {
		strncpy(raop_rtp_mirror->remoteName, remoteName,
			min(128, strlen(remoteName)));
	}
	if (remoteDeviceId != NULL) {
		strncpy(raop_rtp_mirror->remoteDeviceId, remoteDeviceId,
			min(128, strlen(remoteDeviceId)));
	}
...
void raop_rtp_init_mirror_aes(raop_rtp_mirror_t *raop_rtp_mirror,
 ...
 * Mirror
 */
static THREAD_RETVAL raop_rtp_mirror_thread(void *arg)
{
	...
	while (1) {
		...
		if (stream_fd == -1 &&
			...
			if (stream_fd == -1) {
				logger_log(
					...
					"raop_rtp_mirror error in accept %d %s",
			...
			if (setsockopt(stream_fd, SOL_TCP, TCP_KEEPIDLE,
				...
				logger_log(
					raop_rtp_mirror->logger, LOGGER_WARNING,
					"raop_rtp_mirror could not set stream socket keepalive time %d %s",
					errno, strerror(errno));
			}
			option = 10;
			if (setsockopt(stream_fd, SOL_TCP, TCP_KEEPINTVL,
				       &option, sizeof(option)) < 0) {
				logger_log(
					raop_rtp_mirror->logger, LOGGER_WARNING,
					"raop_rtp_mirror could not set stream socket keepalive interval %d %s",
					errno, strerror(errno));
			}
			option = 6;
			if (setsockopt(stream_fd, SOL_TCP, TCP_KEEPCNT, &option,
				       sizeof(option)) < 0) {
				logger_log(
					raop_rtp_mirror->logger, LOGGER_WARNING,
		...
		if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds)) {
			...
			while (payload == NULL && readstart < 128) {
				ret = recv(stream_fd, packet + readstart,
					   128 - readstart, 0);
				if (ret <= 0)
					break;
			...
			if (payload == NULL && ret == 0) {
				...
				logger_log(
					raop_rtp_mirror->logger, LOGGER_ERR,
					"raop_rtp_mirror error in header recv: %d",
					errno);
				break;
			}
			...
			unsigned short payload_type =
				byteutils_get_short(packet, 4) & 0xff;
			unsigned short payload_option =
				byteutils_get_short(packet, 6);
			...
			while (readstart < payload_size) {
				// Payload data
				ret = recv(stream_fd, payload + readstart,
					   payload_size - readstart, 0);
				if (ret <= 0)
					break;
				readstart = readstart + ret;
			}
			...
			if (ret == 0) {
				...
				if (errno == EAGAIN || errno == EWOULDBLOCK)
					continue; // Timeouts can happen even if the connection is fine
				logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
					   "raop_rtp_mirror error in recv: %d",
					   errno);
				break;
			}
			...
			if (payload_type == 0) {
				...
				uint64_t ntp_timestamp_raw =
					byteutils_get_long(packet, 8);
				uint64_t ntp_timestamp_remote =
					raop_ntp_timestamp_to_micro_seconds(
						ntp_timestamp_raw, false);
				uint64_t ntp_timestamp =
					raop_ntp_convert_remote_time(
						raop_rtp_mirror->ntp,
						ntp_timestamp_remote);

				uint64_t ntp_now = raop_ntp_get_local_time(
					raop_rtp_mirror->ntp);
				logger_log(
					raop_rtp_mirror->logger, LOGGER_DEBUG,
					"raop_rtp_mirror video ntp = %llu, now = %llu, latency = %lld",
				...
				fwrite(&readstart, sizeof(readstart), 1,
				       file_len);
#endif
				...
				unsigned char *payload_decrypted =
					malloc(payload_size);
				mirror_buffer_decrypt(raop_rtp_mirror->buffer,
						      payload,
						      payload_decrypted,
						      payload_size);
				...
				while (nalu_size < payload_size) {
					int nc_len =
						(payload_decrypted[nalu_size + 0]
						 << 24) |
						(payload_decrypted[nalu_size + 1]
						 << 16) |
						(payload_decrypted[nalu_size + 2]
						 << 8) |
						(payload_decrypted[nalu_size +
								   3]);
					assert(nc_len > 0);
...
#ifdef DUMP_H264
				fwrite(payload_decrypted, payload_size, 1,
				       file);
#endif
				...
				float width_source =
					byteutils_get_float(packet, 40);
				float height_source =
					byteutils_get_float(packet, 44);
				...
				logger_log(
					...
					width_source, height_source, width,
					height);
				...
				h264.sps_size =
					(short)(((payload[6] & 255) << 8) +
						(payload[7] & 255));
				logger_log(raop_rtp_mirror->logger,
					   LOGGER_DEBUG,
					   "raop_rtp_mirror sps size = %d",
					   h264.sps_size);
				h264.sequence_parameter_set =
					malloc(h264.sps_size);
				memcpy(h264.sequence_parameter_set, payload + 8,
				       h264.sps_size);
				h264.number_of_pps = payload[h264.sps_size + 8];
				h264.pps_size =
					(short)(((payload[h264.sps_size + 9] &
						  2040) +
						 payload[h264.sps_size + 10]) &
						255);
				h264.picture_parameter_set =
					malloc(h264.pps_size);
				logger_log(raop_rtp_mirror->logger,
					   LOGGER_DEBUG,
					   "raop_rtp_mirror pps size = %d",
					   h264.pps_size);
				memcpy(h264.picture_parameter_set,
				       payload + h264.sps_size + 11,
				       h264.pps_size);

				if (h264.sps_size + h264.pps_size < 102400) {
					// Copy the sps and pps into a buffer to hand to the decoder
					int sps_pps_len = (h264.sps_size +
							   h264.pps_size) +
							  8;
					...
					memcpy(sps_pps + h264.sps_size + 8,
					       h264.picture_parameter_set,
					       h264.pps_size);
...
void raop_rtp_start_mirror(raop_rtp_mirror_t *raop_rtp_mirror, int use_udp,
			   unsigned short *mirror_data_lport)
{
...
static int raop_rtp_init_mirror_sockets(raop_rtp_mirror_t *raop_rtp_mirror,
					int use_ipv6)
{
	int dsock = -1;
