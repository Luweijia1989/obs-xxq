			      ...
			      int *response_datalen)
{
	...
	const char *airplay_txt =
		dnssd_get_airplay_txt(conn->raop->dnssd, &airplay_txt_len);

	int name_len = 0;
	const char *name = dnssd_get_name(conn->raop->dnssd, &name_len);

	int hw_addr_raw_len = 0;
	const char *hw_addr_raw =
		dnssd_get_hw_addr(conn->raop->dnssd, &hw_addr_raw_len);

	char *hw_addr = calloc(1, 3 * hw_addr_raw_len);
	int hw_addr_len = utils_hwaddr_airplay(hw_addr, 3 * hw_addr_raw_len,
					       hw_addr_raw, hw_addr_raw_len);
	...
	plist_t features_node =
		plist_new_uint((uint64_t)0x1E << 32 | 0x5A7FFFF7);
	...
	plist_t audio_format_0_audio_input_formats_node =
		plist_new_uint(67108860);
	plist_t audio_format_0_audio_output_formats_node =
		plist_new_uint(67108860);
	plist_dict_set_item(audio_format_0_node, "type",
			    audio_format_0_type_node);
	plist_dict_set_item(audio_format_0_node, "audioInputFormats",
			    audio_format_0_audio_input_formats_node);
	plist_dict_set_item(audio_format_0_node, "audioOutputFormats",
			    audio_format_0_audio_output_formats_node);
	...
	plist_t audio_format_1_audio_input_formats_node =
		plist_new_uint(67108860);
	plist_t audio_format_1_audio_output_formats_node =
		plist_new_uint(67108860);
	plist_dict_set_item(audio_format_1_node, "type",
			    audio_format_1_type_node);
	plist_dict_set_item(audio_format_1_node, "audioInputFormats",
			    audio_format_1_audio_input_formats_node);
	plist_dict_set_item(audio_format_1_node, "audioOutputFormats",
			    audio_format_1_audio_output_formats_node);
	...
	plist_dict_set_item(r_node, "keepAliveLowPower",
			    keep_alive_low_power_node);
	...
	plist_dict_set_item(r_node, "keepAliveSendStatsAsBody",
			    keep_alive_send_stats_as_body_node);
	...
	plist_t audio_latencies_0_output_latency_micros_node =
		plist_new_uint(0);
	...
	plist_dict_set_item(audio_latencies_0_node, "outputLatencyMicros",
			    audio_latencies_0_output_latency_micros_node);
	plist_dict_set_item(audio_latencies_0_node, "type",
			    audio_latencies_0_type_node);
	plist_dict_set_item(audio_latencies_0_node, "audioType",
			    audio_latencies_0_audio_type_node);
	plist_dict_set_item(audio_latencies_0_node, "inputLatencyMicros",
			    audio_latencies_0_input_latency_micros_node);
	plist_array_append_item(audio_latencies_node, audio_latencies_0_node);
	plist_t audio_latencies_1_node = plist_new_dict();
	plist_t audio_latencies_1_output_latency_micros_node =
		plist_new_uint(0);
	...
	plist_dict_set_item(audio_latencies_1_node, "outputLatencyMicros",
			    audio_latencies_1_output_latency_micros_node);
	plist_dict_set_item(audio_latencies_1_node, "type",
			    audio_latencies_1_type_node);
	plist_dict_set_item(audio_latencies_1_node, "audioType",
			    audio_latencies_1_audio_type_node);
	plist_dict_set_item(audio_latencies_1_node, "inputLatencyMicros",
			    audio_latencies_1_input_latency_micros_node);
	...
	plist_t displays_0_uuid_node =
		plist_new_string("e0ff8a27-6738-3d56-8a16-cc53aacee925");
	plist_t displays_0_width_physical_node = plist_new_uint(0);
	plist_t displays_0_height_physical_node = plist_new_uint(0);
	plist_t displays_0_width_node = plist_new_uint(1280);
	plist_t displays_0_height_node = plist_new_uint(720);
	plist_t displays_0_width_pixels_node = plist_new_uint(1280);
	plist_t displays_0_height_pixels_node = plist_new_uint(720);
	...
	plist_dict_set_item(displays_0_node, "widthPhysical",
			    displays_0_width_physical_node);
	plist_dict_set_item(displays_0_node, "heightPhysical",
			    displays_0_height_physical_node);
	...
	plist_dict_set_item(displays_0_node, "widthPixels",
			    displays_0_width_pixels_node);
	plist_dict_set_item(displays_0_node, "heightPixels",
			    displays_0_height_pixels_node);
	plist_dict_set_item(displays_0_node, "rotation",
			    displays_0_rotation_node);
	plist_dict_set_item(displays_0_node, "refreshRate",
			    displays_0_refresh_rate_node);
	plist_dict_set_item(displays_0_node, "overscanned",
	...
	logger_log(conn->raop->logger, LOGGER_DEBUG, "INFO len = %d",
		   response_datalen);
			       ...
			       int *response_datalen)
{
	...
	if (transport) {
		logger_log(conn->raop->logger, LOGGER_DEBUG, "Transport: %s",
			   transport);
	...
	if (PLIST_IS_DATA(req_eiv_node) && PLIST_IS_DATA(req_ekey_node)) {
		...
		int ret = fairplay_decrypt(conn->fairplay,
					   (unsigned char *)ekey, aeskey);
		logger_log(conn->raop->logger, LOGGER_DEBUG,
		...
		conn->raop_ntp = raop_ntp_init(conn->raop->logger, conn->remote,
					       conn->remotelen, timing_rport);
		raop_ntp_start(conn->raop_ntp, &timing_lport);

		// name
		char *name = NULL;
		plist_t name_node = plist_dict_get_item(req_root_node, "name");
		plist_get_string_val(name_node, &name);

		char *deviceId = NULL;
		plist_t device_id_node =
			plist_dict_get_item(req_root_node, "deviceID");
		plist_get_string_val(device_id_node, &deviceId);

		conn->raop_rtp = raop_rtp_init(conn->raop->logger,
					       &conn->raop->callbacks,
					       conn->raop_ntp, conn->remote,
					       conn->remotelen, name, deviceId,
					       aeskey, aesiv, ecdh_secret);
		conn->raop_rtp_mirror = raop_rtp_mirror_init(
			conn->raop->logger, &conn->raop->callbacks,
			conn->raop_ntp, conn->remote, conn->remotelen, name,
			deviceId, aeskey, ecdh_secret);

		plist_t res_event_port_node = plist_new_uint(conn->raop->port);
		plist_t res_timing_port_node = plist_new_uint(timing_lport);
		plist_dict_set_item(res_root_node, "timingPort",
				    res_timing_port_node);
		plist_dict_set_item(res_root_node, "eventPort",
				    res_event_port_node);

		logger_log(conn->raop->logger, LOGGER_DEBUG,
			   "eport = %d, tport = %d", conn->raop->port,
			   timing_lport);
	}
	...
	if (PLIST_IS_ARRAY(req_streams_node)) {
		...
		for (int i = 0; i < count; i++) {
			plist_t req_stream_node =
				plist_array_get_item(req_streams_node, i);
			...
			case 110: {
				...
				plist_get_uint_val(stream_id_node,
				...
				if (conn->raop_rtp_mirror) {
					raop_rtp_init_mirror_aes(
						conn->raop_rtp_mirror,
						stream_connection_id);
					raop_rtp_start_mirror(
						conn->raop_rtp_mirror, use_udp,
						&dport);
					logger_log(
						conn->raop->logger,
						LOGGER_DEBUG,
					...
					logger_log(
						...
						"Mirroring not initialized at SETUP, playing will fail!");
					http_response_set_disconnect(response,
								     1);
				}
	...
	plist_to_bin(res_root_node, response_data,
		     (uint32_t *)response_datalen);
	http_response_add_header(response, "Content-Type",
				 "application/x-apple-binary-plist");
}
				       ...
				       int *response_datalen)
{
	...
	if (!strcmp(content_type, "text/parameters")) {
		...
		while (current && (datalen - (current - data) > 0)) {
			...
			if ((datalen - (current - data) >= 8) &&
			    !strncmp(current, "volume\r\n", 8)) {
			...
			for (next = current; (datalen - (next - data) > 0);
			     ++next)
				if (*next == '\r')
					break;

			if ((datalen - (next - data) >= 2) &&
			    !strncmp(next, "\r\n", 2)) {
				if ((next - current) > 0) {
					logger_log(
						conn->raop->logger,
				       ...
				       int *response_datalen)
{
	...
	if (!strcmp(content_type, "text/parameters")) {
		...
		if (data && datastr && conn->raop_rtp) {
			...
			if ((datalen >= 8) &&
			    !strncmp(datastr, "volume: ", 8)) {
				float vol = 0.0;
				sscanf(datastr + 8, "%f", &vol);
				raop_rtp_set_volume(conn->raop_rtp, vol);
			} else if ((datalen >= 10) &&
				   !strncmp(datastr, "progress: ", 10)) {
