...
const char *userpref_get_config_dir()
{
	...
	printf("initialized config_dir to %s\n", __config_dir);
...
int userpref_read_system_buid(char **system_buid)
{
	...
	printf("using %s as %s\n", *system_buid, USERPREF_SYSTEM_BUID_KEY);
	return 0;
}
...
userpref_error_t userpref_get_paired_udids(char ***list, unsigned int *count)
{
	...
	if (!list || (list && *list)) {
		printf("ERROR: The list parameter needs to point to NULL!\n");
...
userpref_error_t userpref_save_pair_record(const char *udid, plist_t pair_record)
{
	...
	res = config_set_device_record(udid, record_data, record_size);
...
{
	int res = config_remove_device_record(udid);
...
static int X509_add_ext_helper(X509 *cert, int nid, char *value)
{
	...
	if (!ex) {
		printf("ERROR: X509V3_EXT_conf_nid(%d, %s) failed\n", nid, value);
		return 0;
	}
...
userpref_error_t pair_record_generate_keys_and_certs(plist_t pair_record, key_data_t public_key)
{
	...
	printf("Generating keys and certificates...\n");
	...
	RSA *pubkey = NULL;
	{
		...
		if (!PEM_read_bio_RSAPublicKey(membp, &pubkey, NULL, NULL)) {
			printf("WARNING: Could not read public key\n");
		}
		BIO_free(membp);
	}
	...
	if (pubkey && dev_cert) {
		...
		if (X509_sign(dev_cert, root_pkey, EVP_sha1())) {
			...
			printf("ERROR: Signing device certificate with root private key failed!\n");
		}
	}
