...
bool D3D11Renderer::CreateRenderer(UINT width, UINT height)
{
	...
	for (int i = 0; i < PIXEL_SHADER_MAX; i++) {
		render_target_[i].reset(new D3D11RenderTexture(d3d11_device_, d3d11_context_));
	}
	...
	UINT cpu_flags = 0;
	UINT misc_flags = D3D11_RESOURCE_MISC_SHARED;
...
bool D3D11Renderer::CreateTexture(int width, int height, PixelFormat format)
{
	...
	for (int i = 0; i < PIXEL_PLANE_MAX; i++) {
		input_texture_[i].reset(new D3D11RenderTexture(d3d11_device_, d3d11_context_));
	}
