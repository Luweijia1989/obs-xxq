...
D3D11RenderTexture::D3D11RenderTexture(ID3D11Device *device, ID3D11DeviceContext *context)
{
...
bool D3D11RenderTexture::InitTexture(UINT width, UINT height, DXGI_FORMAT format, D3D11_USAGE usage,UINT bind_flags, UINT cpu_flags, UINT misc_flags)
{
	...
	if (misc_flags & D3D11_RESOURCE_MISC_SHARED) {
		...
		hr = texture_->QueryInterface(__uuidof(IDXGIResource),
						(void **)&dxgi_res);
		if (FAILED(hr)) {
			goto failed;
		}

		hr = dxgi_res->GetSharedHandle(&d3d11_shared_handle);
		dxgi_res->Release();
		if (FAILED(hr)) {
			goto failed;
		}
...
HANDLE D3D11RenderTexture::GetTextureSharedHandle()
{
	return d3d11_shared_handle;
}
