...
protected:
	...
	static void connected(void *cls, const char *remoteName,
			      const char *remoteDeviceId);
	static void disconnected(void *cls, const char *remoteName,
				 const char *remoteDeviceId);
	...
	static void audio_set_volume(void *cls, void *session, float volume,
				     ...
				     const char *remoteDeviceId);
	static void audio_set_metadata(void *cls, void *session,
				       const void *buffer, int buflen,
				       const char *remoteName,
				       const char *remoteDeviceId);
	static void audio_set_coverart(void *cls, void *session,
				       const void *buffer, int buflen,
				       const char *remoteName,
				       const char *remoteDeviceId);
	...
	static void audio_destroy(void *cls, void *session,
				  const char *remoteName,
				  const char *remoteDeviceId);
	static void video_process(void *cls, raop_ntp_t *ntp,
				  h264_decode_struct *data,
				  const char *remoteName,
				  const char *remoteDeviceId);
	static void log_callback(void *cls, int level, const char *msg);

	static void ap_video_play(void *cls, char *url, double volume,
				  double start_pos);
	static void ap_video_get_play_info(void *cls, double *duration,
					   double *position, double *rate);
