...
raop_rtp_mirror_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (stream_fd == -1 && FD_ISSET(raop_rtp_mirror->mirror_data_sock, &rfds)) {
            ...
            tv.tv_sec = 0;
            tv.tv_usec = 5000;
            if (setsockopt(stream_fd, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof(tv)) < 0) {
            ...
            int option;
            option = 1;
            if (setsockopt(stream_fd, SOL_SOCKET, SO_KEEPALIVE, (const char *)&option, sizeof(option)) < 0) {
        ...
        if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds)) {
            ...
            if (payload_type == 0) {
                ...
                if (h264.sps_size + h264.pps_size < 102400) {
                    ...
                    unsigned char *sps_pps = (unsigned char *)malloc(sps_pps_len * sizeof(unsigned char));
                    sps_pps[0] = 0;
                    sps_pps[1] = 0;
                    sps_pps[2] = 0;
                    sps_pps[3] = 1;
